...
export default class AuthStorageManager {
  ...
  public static async createAuthDirectoryIfNeeded(): Promise<void> {
    try {
      const directory = AsyncFileManager.getDirectoryPath(AuthStorageConstants.DIRECTORY);

      // Always ensure directory exists, even in CI environment
      await AsyncFileManager.ensureDirectoryExists(directory);
  ...
  public static async resolveAuthStateFilePath(shouldResetFile: boolean = false): Promise<string> {
    try {
      ...
      const filePath = AsyncFileManager.resolvePath(AuthStorageConstants.DIRECTORY, fileName);
  ...
  public static async deleteAuthStateDirectory(): Promise<boolean> {
    try {
      const dirPath = AsyncFileManager.getDirectoryPath(AuthStorageConstants.DIRECTORY);

      const exists = await AsyncFileManager.doesDirectoryExist(dirPath);

      if (exists) {
        await AsyncFileManager.removeDirectory(dirPath);
